...
\title{Charge exchange
  ...
  reaction study at 1.75 \GeVc per nucleon by the STRELA spectrometer}
...
neglected. The results of simulation shown in Fig.~\ref{fig:p1p2sim} include all
...
% These ratios depend on the $(d\sigma/dt)|\,_{t=0}$ of the \np reaction
% extracted from published data.



%%%This value agrees with the differential
% cross section $(d\sigma/dt)|\,_{t=0}=(30.2\,\pm\,4.1$) mb$/$(\GeVc)$^{\,2}$
...
%The obtained ratios Eq. \eqref{eq:rnp} and \eqref{eq:rid}, within the
%uncertainties, are consistent with those of 1m HBC \cite{gla08},
%determined with considerably large errors.

% The value of the
% \dpchex channel differential cross section
% $(d\sigma/dt)|\,_{t=0}=(30.56\,\pm\,0.48$) mb$/$(\GeVc)$^{\,2}$ has been
% established at 1.75 A \GeVc per nucleon. This value agrees with the differential
% cross section $(d\sigma/dt)|\,_{t=0}=(30.2\,\pm\,4.1$) mb$/$(\GeVc)$^{\,2}$ and
% the obtained ratios Eq. \eqref{eq:rnp} and \eqref{eq:rid}, within the
% uncertainties, are also consistent with the results of 1m HBC \cite{gla08},
% where the errors are large.


% The determined
% ratio of the charge exchange differential cross sections at $t=0$ for \dpchex
% to \np reactions
% $R_{\np} = 0.64 \pm 0.01\,\mathrm{(stat.)} \pm 0.04\,\mathrm{(syst.)}$ testifies
% the prevailing contribution of the spin-dependent part to the \np scattering.
% The obtained contribution of the spin-independent part
% $R^{\,ID}_{\np} = 0.05 \pm 0.02\,\mathrm{(stat.)} \pm 0.07\,\mathrm{(syst.)}$
% and the ratio $R_{\np}$ are within the uncertainties consistent with the results
% of 1m HBC \cite{gla08}. Both ratios depend on the $(d\sigma/dt)|\,_{t=0}$ of the
% \np reaction extracted from published data.
