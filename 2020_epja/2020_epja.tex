...
deuteron beams of 3.35 \GeVc momenta, measuring the differential cross section
...
(Fig.~\ref{fig:res_chambers}). The minimum time resolution between consecutive
signals is $\sim$~50 ns, which corresponds to a minimum distance of $\sim$~2 mm
between tracks. This resolution fully satisfies the requirement of the STRELA
experiment. The analyzing magnet spreads in space the recorded protons from the
examined reaction. More technical details and the algorithm of the track
reconstruction can be found in \cite{gla13}.
...
The acceptance of the experimental facility for the studied process of charge
exchange reaction is close to 100~\%.
% Co mozno vidiet aj z obrazku
(see Fig. \ref{fig:p1vsp2}). The picture shown two dimensional distribution
% namiesto distribution -> plot (distribution .. the sentence is very awkward ?)
of the measured momenta $p_1$ \textit{vs.} $p_2$ of the two protons from $dp$
reaction (Fig. \ref{fig:p1vsp2} (b)). Simulated distributions using real data
from one meter HBC at the momenta 1.675 \GeVc are shown in Fig. \ref{fig:p1vsp2}
(a) (all channels $dp$ reaction) and Fig. \ref{fig:p1vsp2} (c) (only \dpfrag
channel). By comparing experimental data with simulation it can be seen that
two protons with momenta approximately equal to the half of the primary deuteron
beam (1.75 \GeVc) of the charge exchange reaction
% \dpchex % vsade piseme dp->ppn (bez zatvoriek)
are completely detected.
% A teda akceptancia je ~100% (na zaciatku alebo nakonci) toho odstavca???
% a tak isto tu vidime, ze pozadie z inych kanalov je zanedbatelne (***)???

% presunut komplet cely obrazok 5 (precislovat) na toto miesto.
% Detto pokec okolo obr.5 (nizsie) v texte je akoby umelo vnoreny.
% ALEEEEE je tu aj veta, ze pozadie z inych kanalov je zanedbatelne a teda
% spomenut to uz tu alebo potom na tom mieste, kde je teraz (***)?

% a nakoniec cely tento odstavec presunt az za nasledujucim odstavcom za
% magnetom ??? alebo este lepsie az za nasledujecim odstavcom (ohaldom oziarenia
% aparatury) pred odstavec ohladom tercikov. Pretoze az tam definujem rozdiel
% medzi dp->(pp)n a dp->(pn)p = dp-> ppn !!! a toto cele presunut na zaciatok
% ALEBO koniec tejto kapitoly (za prvym odstavcom) !!!!!!!!!!!!!!!
...
% (b), it can be seen that the background from other channels of the $dp$
% reaction may be eliminated.
...
% mozno spomenut, ze ten druhy proton (s malym impulsom) sa do nasich kamier
% nedostane (magnet ho vyhodi)
% PREVERIT mame napisane, ze ked su dva protony, tak su to len protony
% z charge exchange, tak mam geometriu nastavenu
%
% !!!!!!!!!!!!!!!!!!!!! mozno dat odstavec ohladom akceptnacie tu !!!!!!!!!!
% a cele to potom presunt na zaciatok ALEBO koniec kapitoly ?!
...
$\sim 10^{6}$ particles per spill, a steel collimator with a rectangular
aperture of 4 $\times$ 4 mm$^2$ and a length of 1.2 m has to be used to reduce
the intensity. After applying the collimator the deuteron beam intensity of
...
deuteron beam. The value of the correction is different from run to run and is
in the interval 0.85~--~0.89. The track of deuteron beam in drift chambers DC1,
DC2 before magnet and behind DC3, DC4 are reconstructed using the track
reconstruction algorithm \cite{gla13}.
...
target is (4.74 $\pm$ 0.05)$\times$10$^{23}$ cm$^{-2}$. The background from
other channels of the $dp$ reaction and the influence of carbon nuclei have been
estimated by the use of the GEANT3 simulation package for transporting the
reaction products (taken from the corresponding events of the one meter bubble
chamber at the momenta 3.35 \GeVc) through the experimental setup. The
background from C and CH$_2$ targets can be neglected. More details about the
...
for both targets are obtained. The dependencies of the sum of the two proton
...
negligible (Fig. \ref{fig:p1p2sim} (b)). For further analysis only those events
...
% alebo to dat az za fit, ma zmysel uvadzat co je N ?
...
\begin{equation}
  ...
  dN/dt = a\,\exp(b\,t)\,,
\end{equation}
with parameters $a=(435.6\,\pm\,6.8)$ and $b=(-440.9\,\pm\,9.1)$. The fitting
% where parameter $a$ is equal the number of ?events?
function is completely empirical.

% TU PRESUNUT VZOREC NA PREPOCET dN/dt na dSigma/dt

The value $(dN/dt)|\,_{t=0}=(435.6\,\pm\,6.8)$
N$/$(\GeVc)$^{\,2}$ corresponds to the charge exchange reaction differential
cross section $(d\sigma/dt)|\,_{t=0}=(30.56\,\pm\,0.48$) mb$/$(\GeVc)$^{\,2}$.
