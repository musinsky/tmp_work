...
spectrometer, consisting of scintillator detectors (S1 -- S3) used to trigger
the setup, blocks of drift chambers (DC1 -- DC4) used as coordinate detector,
...
% nemame definiciu track residual (ale mame linku na publikaciu)
% nechat ako je bez zmeny
\begin{figure}[t]
  \centering
...
form the magnet and from the drift chambers. Into the analy\-sis only the events
...
magnet.
% two (fast) protons
% or a single (slow OR slowly) proton (mozno pisat aj bez zatvoriek)
%
%
% dp->ppn vs dp->(pp)n pri simulaciach VVG ?! pravda je tak aj tak
% nechat ako to je ??? (aj ked to posobi mierne zmatujuco)
...
extrapolation of the measured data to $|t|\mapsto0$. This can only be connected
according to Eq.~\eqref{eq:dp_23np} with the spin-dependent part of the \np
process.
% which can be done => which can only be done ???
% This can only be connected => This can be connected ???
...
% ano, ano, nase exp. data jednoducha exponenta a Bizzarda "dvojita" exponenta
% co (v pripade potreby) mozeme zdvovodnit, ze pri Bizardu bol brany vacsi
% interval (co ale nie je pravda, Bizard fitivany na intevale 0 - 0.01),
% ale Bizard data existuju aj pre |t|> 0.01
% + fitovacie funkcie => systematika
...
\end{equation}
and accordingly the ratio of the two parts of the elastic \np charge exchange
cross section has been obtained as
%
% ?! ale ?! v povodnej vete hovorime spin-independent, R^{ID}
% and accordingly the value of the spin-independent part of the elastic \np
% charge exchange cross section has been obtained as
%
% accordingly the ratio of the two parts (value of the spin-independent part) of
%
% accordingly the value of the spin-independent part (ratio of the two parts) of
\begin{equation}
  \begin{split}
...
obtained by means of the one meter hydrogen bubble chamber at 1.675 A \GeVc per
