...
form the magnet a and from the drift chambers. % DC3 and DC4. The detailed track
% reconstruction algorithm can be found in \cite{gla13}. (citacia je uz skor)
...
The deuteron breakup events reaching the drift chambers DC3 and DC4 are supposed
to contain: two protons with close momenta, approximately equal to the half of
the incident deuteron beam momenta, from the charge exchange reaction \dpchex,
or a single proton from the charge retention \dpfrag channel, where the recoil
proton is filtered out by the magnet.
...
one compare with the results of \dpchex simulation in STARONOVYb. This
crosschecks the statements made about the detector acceptance above. The
arch like areas in STARONOVYa) and NOVY correspond to none charge exchange to
two-track events. A simple cut on the sum of the two reconstructed momenta can
remove the events from the arch like area.

For further analysis only those events have been
...
% URBAN nova veta
%In the measured data (Fig. novy) two well-seperated areas can be distinguished
%as well as in the simulated ones (Fig. 5a).

%The more populated ellipse like area in
%Fig. 5 c can be ascribed to the charge exchange events if one compare with the
%results of dp->(pp)n simulation in Fig. 5b. From this comparison one can also
%judge that the two protons of the charge exchange reaction arefully in the
%detector acceptance.
